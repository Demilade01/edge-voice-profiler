...
export class LatencyLogger {
  ...
  private formatEvent(event: LatencyEvent): string {
  ...
  private logLatencyBreakdown(turn: ConversationTurn): void {
    console.log('\n┌─────────────────────────────────────────────────────┐');
  ...
  private calculateBreakdown(turn: ConversationTurn): { [key: string]: number } {
  ...
  private logToConsole(level: 'info' | 'event' | 'error', message: string): void {
  ...
  exportToJSON(): string {
    return JSON.stringify(this.getAllTurns(), (key, value) =>
